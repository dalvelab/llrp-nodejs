...
exports.encodeParameter = exports.encodeMessage = void 0;
const encodeMessage = function (llrpMessage) {
    // Create the buffer, should be the length of the message.
    // We will not account for slicing the data.
    const buffer = Buffer.alloc(llrpMessage.getLength());
...
const encodeParameter = function (llrpParameter) {
    ...
    if (llrpParameter.getType() < 128) {
        ...
        buffer.writeUInt8((llrpParameter.getReserved() << 2) |
            ((llrpParameter.getType() & 768) >> 8), 0);
