// Constants
import { rospecConstants } from "./constants/roSpecConstants";

// Encoder
import { encodeMessage, encodeParameter } from "./actions/encode";
...
import { LLRPParameter } from "./parameterLLRP";

// Interfaces
import { LlrpMessage, Message } from "./interfaces/llrpMessageInterface";
import { Parameter } from "./interfaces/llrpParametersInterface";
import { RospecParameters, AntennaConfig } from "./interfaces/llrpInterface";

const vendorId: number = rospecConstants.impinjVendorId;

const MAX_RFID_POWER_FOR_THE_EU: number = 31.5;

export class getMessageLLRP {
  public static resetConfigurationToFactoryDefaults(): Buffer {
    return this.makeMessage(
      MessagesType.SET_READER_CONFIG,
      1205,
      Buffer.from([0x80])
    );
  }
  ...
  public static startRoSpec(roSpecID: number): Buffer {
    return this.makeMessage(
      MessagesType.START_ROSPEC,
      0,
      this.getUint32Parameter(roSpecID)
    );
  }

  public static deleteAllAccessSpec(): Buffer {
    return this.makeMessage(
      MessagesType.DELETE_ACCESSSPEC,
      0,
      this.getUint32Parameter(0)
    );
  }

  public static deleteAllROSpecs(): Buffer {
    return this.makeMessage(
      MessagesType.DELETE_ROSPEC,
      0,
      this.getUint32Parameter(0)
    );
  }
  ...
  public static enableExtensions(): Buffer {
    ...
    return this.makeMessage(
      MessagesType.CUSTOM_MESSAGE,
      1,
      enableExtensionsParams
    );
  }
  ...
  public static setReaderConfig(): Buffer {
    ...
    const eventsAndReports: Buffer = this.makeTLVParameter(
      ParameterConstants.EventsAndReports,
    ...
    return this.makeMessage(
      MessagesType.SET_READER_CONFIG,
      0,
      Buffer.concat([setReaderConfigParameter, eventsAndReports])
    );
  }

  public static addRoSpec(
    roSpecID: number,
    parameters: RospecParameters
  ): Buffer {
    const rOSpecStartTriggerParams: Buffer = Buffer.allocUnsafe(5);
    rOSpecStartTriggerParams.writeUInt8(
      rospecConstants.ROSpecStopTriggerType,
      0
    );
    rOSpecStartTriggerParams.writeUInt32BE(
      rospecConstants.DurationTriggerValue,
      1
    );

    const rOSpecStartTrigger: Buffer = this.makeTLVParameter(
      ParameterConstants.ROSpecStartTrigger,
      Buffer.from([rospecConstants.ROSpecStartTriggerType])
    );
    const rOSpecStopTrigger: Buffer = this.makeTLVParameter(
      ParameterConstants.ROSpecStopTrigger,
      rOSpecStartTriggerParams
    );
    const totalLengthRo: number =
      rOSpecStartTrigger.length + rOSpecStopTrigger.length;
    const rOBoundarySpec: Buffer = this.makeTLVParameter(
      ParameterConstants.ROBoundarySpec,
      Buffer.concat([rOSpecStartTrigger, rOSpecStopTrigger], totalLengthRo)
    );
    ...
    const сonfigEachAntenna: boolean = !!(
      parameters.antennasConfig && antennaCount
    );
    ...
    aiSpecStopTriggerParams.writeUInt8(
      rospecConstants.AISpecStopTriggerType,
      0
    );
    aiSpecStopTriggerParams.writeUInt32BE(
      rospecConstants.AISpecStopDurationTriggerValue,
      1
    );
    const aiSpecStopTrigger: Buffer = this.makeTLVParameter(
      ParameterConstants.AISpecStopTrigger,
      aiSpecStopTriggerParams
    );

    let antennaConfiguration: Buffer = getMessageLLRP.getAntennaConfig(
      0,
      this.getAntennaTransmitPowerIndex(MAX_RFID_POWER_FOR_THE_EU),
      1,
      1,
      3,
      4
    ...
    const inventoryParameterSpecParams: Buffer = Buffer.allocUnsafe(
      3 + antennaConfiguration.length * antennaCount
    );
    inventoryParameterSpecParams.writeUInt16BE(
      rospecConstants.inventoryParameterSpecId,
      0
    );
    ...
    if (сonfigEachAntenna) {
      parameters.antennasConfig.forEach(
        (config: AntennaConfig, index: number) => {
          antennaConfiguration = getMessageLLRP.getAntennaConfig(
            config.number,
          ...
          antennaConfiguration.copy(
            inventoryParameterSpecParams,
            index * antennaConfiguration.length + 3
          );
        }
      );
    }

    const inventoryParameterSpec: Buffer = this.makeTLVParameter(
      ParameterConstants.InventoryParameterSpec,
      inventoryParameterSpecParams
    );

    const aiSpec: Buffer = this.makeTLVParameter(
      ParameterConstants.AISpec,
      Buffer.concat([aiSpecParams, aiSpecStopTrigger, inventoryParameterSpec])
    );
    ...
    c1G2EPCMemorySelectorParam.writeUInt8(
      rospecConstants.enableCRC | rospecConstants.enablePCbits,
      0
    );
    const c1G2EPCMemorySelector: Buffer = this.makeTLVParameter(
      ParameterConstants.C1G2EPCMemorySelector,
      c1G2EPCMemorySelectorParam
    );

    const tagReportContentSelectorBits: number =
      rospecConstants.enableROSpecID |
    ...
    tagReportContentSelectorParameter.writeUInt16BE(
      tagReportContentSelectorBits,
      0
    );
    const tagReportContentSelectorParameters: Buffer = Buffer.concat([
      tagReportContentSelectorParameter,
      c1G2EPCMemorySelector,
    ]);
    const tagReportContentSelector: Buffer = this.makeTLVParameter(
      ParameterConstants.TagReportContentSelector,
      tagReportContentSelectorParameters
    );
    ...
    enableSerializedTidParams.writeUInt32BE(
      impinjSubParameterEnableSerializedTid,
      4
    );
    enableSerializedTidParams.writeUInt16BE(serializedTidMode, 8);
    const enableSerializedTid: Buffer = this.makeTLVParameter(
      ParameterConstants.Custom,
      enableSerializedTidParams
    );
    ...
    const customParamImpinjTagReportContentSelector: Buffer = this.makeTLVParameter(
      ParameterConstants.Custom,
      tagReportContentSelectorParams
    );
    ...
    const rOReportSpec: Buffer = this.makeTLVParameter(
      ParameterConstants.ROReportSpec,
      Buffer.concat(paramsBuffer)
    );
    ...
    const addRospecData: Buffer = this.makeTLVParameter(
      ParameterConstants.ROSpec,
      Buffer.concat([rOSpecIDParameters, rOBoundarySpec, aiSpec, rOReportSpec])
    );

    const answer: Buffer = this.makeMessage(
      MessagesType.ADD_ROSPEC,
      4,
      addRospecData
    );
  ...
  private static makeTLVParameter(
  ...
  private static getAntennaTransmitPowerIndex(
  ...
  static getAntennaConfig(
    antennaId: number,
    transmitPowerValue: number,
    channelIndex: number,
    inventorySearchMode: number,
    modeIndex: number,
    ...
    const rfReceiver: Buffer = this.makeTLVParameter(
      ParameterConstants.RFReceiver,
      rfReceiverParam
    );
    ...
    const rfTransmitter: Buffer = this.makeTLVParameter(
      ParameterConstants.RFTransmitter,
      rfTransmitterParams
    );
    ...
    const c1g2RfControl: Buffer = this.makeTLVParameter(
      ParameterConstants.C1G2RFControl,
      c1g2RfControlParams
    );
    ...
    c1g2SingulationControlParams.writeUInt32BE(
      rospecConstants.tagTranzitTime,
      3
    );
    const c1g2SingulationControl: Buffer = this.makeTLVParameter(
      ParameterConstants.C1G2SingulationControl,
      c1g2SingulationControlParams
    );
    ...
    const customParamImpinjInventorySearchMode: Buffer = this.makeTLVParameter(
      ParameterConstants.Custom,
      inventorySearchModeParams
    );

    const c1g2InventoryCommandParam: Buffer = Buffer.allocUnsafe(1);
    c1g2InventoryCommandParam.writeUInt8(
      rospecConstants.tagInventoryStateAwareNo,
      0
    );
    ...
    const c1g2InventoryCommand: Buffer = this.makeTLVParameter(
      ParameterConstants.C1G2InventoryCommand,
      Buffer.concat(paramsBuffer)
    );

    const antennaConfigurationParam: Buffer = Buffer.allocUnsafe(2);
    antennaConfigurationParam.writeUInt16BE(antennaId, 0);
    const antennaConfiguration: Buffer = this.makeTLVParameter(
      ParameterConstants.AntennaConfiguration,
      Buffer.concat([
